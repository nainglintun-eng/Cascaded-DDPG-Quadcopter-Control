"""
generate_quadcopter_video.py

Generates a video visualization of the trained cascaded DDPG quadcopter
following the expanding helix trajectory.
"""

import numpy as np
import matplotlib.pyplot as plt
from matplotlib.animation import FuncAnimation  # Fixed: was 'Funcanimation'
...
class QuadcopterVisualizer:
    """Generates video visualization of quadcopter trajectory simulation"""
    
    def __init__(self, position_agent_path, attitude_agent_path=None, output_dir='videos'):
        self.sys_cfg = SystemConfig()
        self.pos_cfg = PositionControllerConfig()
        
        # Load agents
        print("Loading trained agents...")
        self.position_agent = self._load_position_agent(position_agent_path)
        
        if attitude_agent_path and os.path.exists(attitude_agent_path):
            self.attitude_agent = self._load_attitude_agent(attitude_agent_path)
        else:
            self.attitude_agent = None
            print("  ⚠ No attitude agent loaded - using PID fallback")
            
        # Create environment
        self.env = PositionEnv(attitude_agent=self.attitude_agent)
    ...
    def _load_position_agent(self, path):
        """Load position controller"""
        agent = DDPGAgent(
            state_dim=self.pos_cfg.STATE_DIM,
            action_dim=self.pos_cfg.ACTION_DIM,
            max_action=self.pos_cfg.MAX_BODY_ACCELERATION,
            hidden_dims=self.pos_cfg.HIDDEN_DIMS
        )
        ...
        if os.path.exists(path):
            try:
                ...
                print(f"  ✓ Loaded position agent from: {path}")
            except Exception as e:
                print(f"  ⚠ Could not load position agent: {e}")
        else:
            print(f"  ⚠ Position agent not found: {path}")
            
        return agent
    
    def _load_attitude_agent(self, path):
        """Load attitude controller"""
        from configs.config import AttitudeControllerConfig
        ...
        try:
            ...
            agent.actor.load_state_dict(checkpoint['actor_state_dict'], strict=False)
            agent.actor_target.load_state_dict(checkpoint['actor_target_state_dict'], strict=False)
            print(f"  ✓ Loaded attitude agent from: {path}")
        except Exception as e:
            print(f"  ⚠ Could not load attitude agent: {e}")
    ...
    def run_simulation(self, duration=50.0):
        ...
        while time < duration and step_count < max_steps:
            # Get action from position controller
            action = self.position_agent.select_action(obs, explore=False)
            ...
            # Calculate attitude error (if desired angles available)
            att_des = info.get('att_des', np.zeros(3))
            if isinstance(att_des, np.ndarray):
                att_err = np.linalg.norm(np.radians(att_des) - att)
            else:
                att_err = 0
            ...
            self.thrusts.append(info.get('thrust', 0))
            self.torques.append(info.get('torques', np.zeros(3)))
            self.times.append(time)
            self.errors.append(error)
            self.attitude_errors.append(att_err)
            self.angular_rates.append(self.env.state[9:12].copy())
            
            # Progress indicator
            if step_count % 1000 == 0:
                print(f"  Progress: {time:.1f}s, Error: {error*1000:.1f}mm")
    ...
    def create_3d_animation(self, filename='quadcopter_trajectory.mp4', 
        ...
        ax.set_title('Cascaded DDPG Quadcopter Trajectory Tracking\nExpanding Helix Mission', 
        ...
        def animate(frame):
            ...
            text_box.set_text(f'Time: {t:.1f}s\n'
    ...
    def create_dashboard_animation(self, filename='quadcopter_dashboard.mp4',
        ...
        ax_3d.set_title('Cascaded DDPG Quadcopter Trajectory Tracking', fontweight='bold', fontsize=11)
        ...
        ax_att.set_title('Attitude', fontweight='bold', fontsize=10)
        ...
        ax_err.set_title('Tracking Error', fontweight='bold', fontsize=10)
        ...
        ax_thrust.set_title('Total Thrust', fontweight='bold', fontsize=10)
        ...
        ax_thrust.axhline(y=self.sys_cfg.MASS * self.sys_cfg.GRAVITY, 
                         color='k', linestyle='--', alpha=0.5, linewidth=0.8)
        ...
        ax_torques.set_title('Control Torques', fontweight='bold', fontsize=10)
        ...
        def animate(frame):
            ...
            text_str = (f"SIMULATION TELEMETRY\n"
                       f"{'='*25}\n"
            ...
            return (*drone_arm_lines3d, *drone_rotor_lines3d, *drone_body_lines3d,
                   ...
                   err_line, thrust_line, torque_x_line, torque_y_line, torque_z_line,
                   ax_text)
...
def main():
    """Main function to generate videos from trained agents"""
    
    # Configuration
    # Update these paths to your trained model files
    POSITION_AGENT_PATH = r'weights\position_best.pth'  # Update with your actual path
    ATTITUDE_AGENT_PATH = r'weights\attitude_best.pth'  # Optional: update if you have an attitude agent
    DURATION = 50.0  # seconds
    OUTPUT_DIR = 'videos'
    
    print("\n" + "="*70)
    print("QUADCOPTER VIDEO GENERATION")
    print("="*70)
    
    # Check if model files exist
    if not os.path.exists(POSITION_AGENT_PATH):
        print(f"\n⚠ Warning: Position agent not found at: {POSITION_AGENT_PATH}")
        print("Available weight files in results directory:")
        results_dir = os.path.dirname(POSITION_AGENT_PATH)
    ...
    # Create visualizer
    visualizer = QuadcopterVisualizer(
        position_agent_path=POSITION_AGENT_PATH,
        attitude_agent_path=ATTITUDE_AGENT_PATH,
