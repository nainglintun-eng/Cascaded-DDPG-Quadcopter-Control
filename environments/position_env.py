    ...
    def reset(self, initial_pos=None):
        ...
        if initial_pos is None:
            if self.sys_cfg.RANDOM_START and self.start_radius > 0:
                ...
                # use beta distribution to bias towards starting far from the origin, but allow some outliers near start_radius
                radius     = np.random.beta(a=2.0, b=1.0) * self.start_radius
                #radius      = np.random.uniform(0.05, self.start_radius)
